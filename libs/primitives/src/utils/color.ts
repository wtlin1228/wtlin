...
export function alpha(value: Value, amount: number) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return (obj: any) =>
    transparentize(resolveValue(value, obj), 1 - amount).replace(/ /g, '');
}

export function lighten(value: Value, amount: number) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return (obj: any) =>
    _lighten(resolveValue(value, obj), amount).replace(/ /g, '');
}

export function darken(value: Value, amount: number) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return (obj: any) =>
    _darken(resolveValue(value, obj), amount).replace(/ /g, '');
}
